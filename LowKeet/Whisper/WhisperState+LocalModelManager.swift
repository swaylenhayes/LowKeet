...
extension WhisperState {
    ...
    func loadAvailableModels() {
        ...
        // OFFLINE MODE: Check bundled models first
        if let bundledDir = bundledModelsDirectory {
            do {
                let bundledFiles = try FileManager.default.contentsOfDirectory(at: bundledDir, includingPropertiesForKeys: nil)
                let bundledModels = bundledFiles.compactMap { url -> WhisperModel? in
                ...
                logInfo("Loaded \(bundledModels.count) models from app bundle")
            } catch {
                logError("Error loading bundled models", error)
            }
        }

        // Then check Application Support directory
        do {
            let fileURLs = try FileManager.default.contentsOfDirectory(at: modelsDirectory, includingPropertiesForKeys: nil)
            let appSupportModels = fileURLs.compactMap { url -> WhisperModel? in
                guard url.pathExtension == "bin" else { return nil }
                // Skip if already loaded from bundle
                let modelName = url.deletingPathExtension().lastPathComponent
                if models.contains(where: { $0.name == modelName }) {
                    return nil
                }
                return WhisperModel(name: modelName, url: url)
            }
            models.append(contentsOf: appSupportModels)
        } catch {
            logError("Error loading available models from Application Support", error)
        }
    ...
    func downloadModel(_ model: LocalModel) async {
        ...
        if isOfflineMode {
            logInfo("Model download disabled in offline mode - all models are pre-bundled")
            return
        }
